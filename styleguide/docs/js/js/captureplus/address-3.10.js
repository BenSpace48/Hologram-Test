...
(function (window, undefined) {
    ...
    ///<summary>Container for page elements.</summary>
    pca.container = null;
    ...
    pca.AutoComplete = function (fields, options) {
        ...
        autocomplete.attach = function (field) {
            ...
            function anchorToField(f) {
                var anchor = pca.create("table", { className: "pca anchor", cellPadding: 0, cellSpacing: 0 }),
        ...
        ///<summary>Shows the autocomplete list under a field.</summary>
        ...
        autocomplete.handleKey = function (key, event) {
            if (key == 27) { //escape
                ...
                if (autocomplete.visible && autocomplete.list.navigate(key) && event)
        ...
        autocomplete.keyup = function (event) {
            autocomplete.handleKey(window.event ? window.event.keyCode : event.which, event);
        }
        ...
        autocomplete.filter = function (term) {
            ...
            autocomplete.list.filter(term);

            term && !autocomplete.list.collection.count ? autocomplete.hide() : autocomplete.show();

            return autocomplete;
        }

        ///<summary>Disables the autocomplete.</summary>
        autocomplete.disable = function () {
            autocomplete.disabled = true;
        }

        ///<summary>Enables the autocomplete when disabled.</summary>
    ...
    ///<summary>Creates a helpful tooltip when hovering over an element.</summary>
    pca.Tooltip = function (element, message) {
        var tooltip = new pca.Object(this);

        tooltip.element = pca.create("div", { className: "tooltip" });
        tooltip.background = pca.create("div", { className: "background" });
        tooltip.message = pca.create("div", { className: "message", innerText: message });
    ...
    ///<summary>Adds an element to the container on the page.</summary>
    ...
    ///<summary>Removes an element from the container on the page.</summary>
    pca.remove = function (element) {
        if (element) pca.container.removeChild(element);
    }
    ...
    pca.Address = function (fields, options) {
        ...
        address.translations = {
            "fr": {
                ...
                NOADDRESS: "Désolé, nous n'avons pas trouvé cette adresse",
            ...
            "de": {
                ...
                NORESULTS: "Leider wurden keine Ergebnisse gefunden",
                NOADDRESS: "Leider können wir nicht finden, diese-Adresse",
        ...
        address.load = function () {
            ...
            var link = pca.create("a", { href: (address.options.brand == "CanadaPost" ? "http://www.canadapost.ca/pca" : "http://www.postcodeanywhere.co.uk"), target: "_blank" }, "display:none;"),
                logo = pca.create("div", { className: (address.options.brand == "None" ? "" : address.options.brand == "CanadaPost" ? "aclogo" : "pcalogo") });
        ...
        ///<summary>Searches based upon the content of the current field.</summary>
        address.searchFromField = function () {
            var term = pca.getValue(address.autocomplete.field);

            if (address.countrylist.autocomplete.visible)
                address.countrylist.autocomplete.filter(term);
            else if (term && !address.initialSearch && term.length >= address.options.minSearch) {
                address.initialSearch = true;
                address.search(term);
            }
        }

        ///<summary>Takes a search string and gets matches for it.</summary>
        address.search = function (term) {

            function success(response) {
                if (response.length)
                    address.display(response, address.templates.AUTOCOMPLETE);
                else
            ...
            if (term)
                pca.fetch("CapturePlus/Interactive/Find/v1.00", { Key: address.key, Country: address.country, SearchTerm: term, SearchFor: address.searchMode.EVERYTHING, $block: true, $cache: true }, success, address.error);
        ...
        address.searchByLocation = function (latitude, longitude) {
            ...
            function fetchLocation() {
                ...
                pca.fetch("CapturePlus/Interactive/FindByPosition/v1.00", { Key: address.key, Latitude: latitude, Longitude: longitude, LocationAccuracy: accuracy, $block: true, $cache: true }, success, address.error);
            }
        ...
        ///<summary>Retrieves an address from an Id and populates the fields.</summary>
        address.retrieve = function (id) {
            ...
            pca.fetch("CapturePlus/Interactive/Retrieve/v1.00", { Key: address.key, Id: id, Source: address.options.source }, success, address.error);
        }
        ...
        address.display = function (results, template) {
        ...
        address.select = function (suggestion) {
            ...
            suggestion.Next == "Retrieve" ? address.retrieve(suggestion.Id) : address.search(suggestion.Id);
            return address;
        }

        ///<summary>Adds highlights to suggestions</summary>
        ///<remarks>HTML bold tags by default.</remarks>
        address.highlight = function (suggestions, prefix, suffix) {
            ...
            for (var s = 0; s < suggestions.length; s++) {
                ...
                for (var i = highlights.length - 1; i >= 0; i--) {
                    ...
                    suggestion.Text = suggestion.Text.substring(0, parseInt(indexes[0])) + prefix + suggestion.Text.substring(parseInt(indexes[0]), parseInt(indexes[1])) + suffix + suggestion.Text.substring(parseInt(indexes[1]), suggestion.Text.length);
                }
            }
        }

        ///<summary>Populate the fields with the address result.</summary>
        address.populate = function (response) {
            ...
            for (var a = 0; a < address.fields.length && address.options.populate; a++) {
                ...
                var value = /[\{\}]/.test(field.field) ? pca.formatLine(details, field.field) : details[field.field];
        ...
        ///<summary>Switches to the country list.</summary>
        ...
        ///<summary>Switches back to the default search list.</summary>
        ...
        ///<summary>Sets the control language.</summary>
        address.detectLanguage = function () {
            if (!window || !window.navigator || !window.navigator.language) return;

            var language = address.options.culture || window.navigator.language,
                languageCode = language && language.length > 1 ? language.substring(0, 2).toLowerCase() : "en";

            if (address.translations[languageCode])
                address.messages = address.translations[languageCode];
        }
        ...
        ///<summary>Hides the address control.</summary>
